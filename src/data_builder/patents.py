# src/data_builder/patents_biblio.py
...
class PatentBiblioClient:
    ...
    def _parse_biblio_detail_xml(self, xml_text: str) -> dict:
        ...
        # Publication date (simple version)
        # OPS biblio XML often has a publication-reference with a date:
        # <publication-reference> ... <date>YYYYMMDD</date> ...
        pub_date_text = ""
        pub_dates = root.findall(f".//{{{self.EX_NS}}}date")
        for d in pub_dates:
            # take the first date we see; you can refine later
            if d.text:
...
###----------Helper function------------------------------
    ...
    def fetch_biblio_last_n_years(self, years_back: int, keywords: List[str] | None = None) -> pd.DataFrame:
        ...
        # 1. Search to get publication references
        xml_search = self._request_search(q)
        df_refs = self._parse_publication_refs(xml_search)

        # Optional: sample to avoid too many biblio calls
        # df_refs = df_refs.head(50)

        # 2. Enrich each reference with title + abstract
        titles = []
        abstracts = []
        pub_dates = []
